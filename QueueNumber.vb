...
Public Class QueueNumber
    ...
    Private Sub qEnqueue_Click(sender As Object, e As EventArgs) Handles qEnqueue.Click
        ...
        Query = "SELECT MAX(queueno) FROM queue.window1"
        ...
        digit = digit + 1
        result = "A" + digit.ToString("D3")
        BunifuCustomLabel1.Text = result
        con.Close()

        con.Open()
        Dim Query2 As String
        Query2 = "INSERT INTO queue.window1 (queueno) VALUES ('" & result & "')"
        ...
        Query4 = "SELECT * FROM queue.window1"
        com = New OdbcCommand(Query4, con)
        dt = New DataTable
        dt.Load(com.ExecuteReader)
        QueueController.Window1Queue.DataSource = dt
        con.Close()

        If QueueController.Window1Queue.Rows.Count > 0 Then
            QueueController.qDequeue.Enabled = True
            QueueController.qVoid.Enabled = True
        End If
    ...
    Private Sub QueueNumber_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        If QueueingWindow.Window1Queue.Rows.Count > 0 Then
            QueueingWindow.Window1Queue.Rows(0).DefaultCellStyle.ForeColor = Color.LimeGreen
            QueueingWindow.Window1Queue.Rows(0).Selected = False
        End If
    End Sub
