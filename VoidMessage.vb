...
Public Class VoidMessage
    ...
    Sub Yes()
        ...
        If QueueController.Window1Queue.Rows.Count > 0 Then
            con.Open()
            Dim Query As String
            Query = "Delete from queue.window1 where queueno='" & QueueController.Window1Queue.SelectedRows(0).Cells(0).Value & "'"
            com = New OdbcCommand(Query, con)
            com.ExecuteNonQuery()
            con.Close()
        End If

        If QueueController.Window1Queue.Rows.Count = 1 Then
            QueueController.qDequeue.Enabled = False
            QueueController.qVoid.Enabled = False
        End If
