...
Public Class VoidMessage2
    ...
    Sub Yes()
        ...
        If QueueController2.Window2Queue.Rows.Count > 0 Then
            con.Open()
            Dim Query As String
            Query = "Delete from queue.window2 where queueno='" & QueueController2.Window2Queue.SelectedRows(0).Cells(0).Value & "'"
            com = New OdbcCommand(Query, con)
            com.ExecuteNonQuery()
            con.Close()
        End If

        If QueueController2.Window2Queue.Rows.Count = 1 Then
            QueueController2.qDequeue.Enabled = False
            QueueController2.qVoid.Enabled = False
        End If
