...
Public Class QueueController3
    ...
    Private Sub QueueController_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Query = "SELECT * FROM queue.window3"
    ...
    Private Sub BunifuFlatButton1_Click(sender As Object, e As EventArgs) Handles qDequeue.Click
        ...
        If Window3Queue.Rows.Count > 0 Then
            con.Open()
            Dim Query As String
            Query = "Delete from queue.window3 where queueno='" & Window3Queue.Rows(0).Cells(0).Value & "'"
            com = New OdbcCommand(Query, con)
            com.ExecuteNonQuery()
            con.Close()
        End If

        If Window3Queue.Rows.Count = 1 Then
            qDequeue.Enabled = False
            qVoid.Enabled = False
        End If
    ...
    Private Sub BunifuImageButton1_Click(sender As Object, e As EventArgs) Handles BunifuImageButton1.Click
        ...
        con.Open()
        Dim query As String
        query = "UPDATE queue.status set window3 = 'OFFLINE' where no = '1'"
        com = New OdbcCommand(query, con)
        com.ExecuteNonQuery()
        InfodeskDashboard.status3.ForeColor = Color.LightCoral
        WindowSelection.status3.ForeColor = Color.LightCoral
        con.Close()
        ...
        com = Nothing
        WindowSelection.status3.Text = statusa
        con.Close()
