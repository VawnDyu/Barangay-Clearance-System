...
Public Class QueueNumber2
    ...
    Private Sub qEnqueue_Click(sender As Object, e As EventArgs) Handles qEnqueue.Click
        ...
        Query = "SELECT MAX(queueno) FROM queue.window2"
        ...
        digit = digit + 1
        result = "B" + digit.ToString("D3")
        BunifuCustomLabel1.Text = result
        con.Close()

        con.Open()
        Dim Query2 As String
        Query2 = "INSERT INTO queue.window2 (queueno) VALUES ('" & result & "')"
        ...
        Query4 = "SELECT * FROM queue.window2"
        com = New OdbcCommand(Query4, con)
        dt = New DataTable
        dt.Load(com.ExecuteReader)
        QueueController2.Window2Queue.DataSource = dt
        con.Close()

        If QueueController2.Window2Queue.Rows.Count > 0 Then
            QueueController2.qDequeue.Enabled = True
            QueueController2.qVoid.Enabled = True
        End If
    ...
    Private Sub QueueNumber_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        If QueueingWindow.Window2Queue.Rows.Count > 0 Then
            QueueingWindow.Window2Queue.Rows(0).DefaultCellStyle.ForeColor = Color.LimeGreen
            QueueingWindow.Window2Queue.Rows(0).Selected = False
        End If
    End Sub
